		...
		if chat_status.RequireUserAdmin(chat, msg, user.Id) {
			if len(args) != 0 {
				...
				case "true", "on", "yes":
					...
					_, err := msg.ReplyHTML(function.GetString(chat.Id, "handlers/setting/setting.go:1"))
					return err
				case "false", "off", "no":
					...
					return err
				default:
					_, err := msg.ReplyHTML(function.GetString(chat.Id, "handlers/setting/setting.go:2"))
					return err
				}
			} else {
				_, err := msg.ReplyHTML(function.GetString(chat.Id, "handlers/setting/setting.go:2"))
	...
	if chat_status.RequireSupergroup(chat, msg) {
		if chat_status.RequireUserAdmin(chat, msg, user.Id) {
			if len(args) != 0 {
				...
				_, err = msg.ReplyHTML(function.GetString(chat.Id, "handlers/setting/setting.go:3"))
				return err
			}
			_, err := msg.ReplyHTML(function.GetString(chat.Id, "handlers/setting/setting.go:3"))
			return err
...
func setLang(b ext.Bot, u *gotgbot.Update, args []string) error {
	...
	if !goloc.IsLangSupported(args[0]) {
		_, err := msg.ReplyHTML(function.GetString(chat.Id, "handlers/language/language.go:58"))
